...
# X and Y frequencies
XY_FREQ = [
    {'name': 'X Freq.', 'config': 'shaper_freq_x', 'min': 0, 'max': 133},
    {'name': 'Y Freq.', 'config': 'shaper_freq_y', 'min': 0, 'max': 133},
]
...
class InputShaperPanel(ScreenPanel):
    def initialize(self, panel_name):
        ...
        manual_calibration_label.set_markup('\n<big><b>Manual Calibration</b></big>')
        ...
        for i, dim_freq in enumerate(XY_FREQ):
            ...
            shaper_grid.attach(scale, 1, 0, 1, 1)

            stype = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
            name = Gtk.Label()
            name.set_markup("<b>{}</b>".format(dim_freq['name'].replace('Freq.', 'Shaper Type')))
            name.set_vexpand(True)
            ...
            stype.add(self.freq_xy_combo[shaper_slug])
            shaper_grid.attach(stype, 2, 0, 1, 1)

            frame.add(shaper_grid)
